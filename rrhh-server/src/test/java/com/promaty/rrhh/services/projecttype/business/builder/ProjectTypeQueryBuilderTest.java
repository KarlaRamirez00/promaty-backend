...
class ProjectTypeQueryBuilderTest {
	...
	private final Root<ProjectType> root = mock(Root.class);
	private final CriteriaQuery<?> query = mock(CriteriaQuery.class);
	private final CriteriaBuilder cb = mock(CriteriaBuilder.class);

	@Test
	void fromFilters_sinFiltros_noAgregaPredicados() {
		ProjectTypeFilterParams filtros = new ProjectTypeFilterParams();
		Predicate combinado = mock(Predicate.class);
		ArgumentCaptor<Predicate[]> captor = ArgumentCaptor.forClass(Predicate[].class);
		when(cb.and(captor.capture())).thenReturn(combinado);

		Predicate resultado = ProjectTypeQueryBuilder.fromFilters(filtros).toPredicate(root, query, cb);
	...
	void fromFilters_conSearch_agregaPredicadoLikeSobreName() {
		...
		when(cb.like(nameLower, "%obra%")).thenReturn(likePredicate);
		ArgumentCaptor<Predicate[]> captor = ArgumentCaptor.forClass(Predicate[].class);
		when(cb.and(captor.capture())).thenReturn(combinado);
	...
	void fromFilters_conSearchEnBlanco_noAgregaPredicado() {
		...
		ProjectTypeQueryBuilder.fromFilters(filtros).toPredicate(root, query, cb);
