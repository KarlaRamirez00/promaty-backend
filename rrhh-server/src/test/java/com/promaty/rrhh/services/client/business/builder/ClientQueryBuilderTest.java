...
class ClientQueryBuilderTest {

	private final Root<Client> root = mock(Root.class);
	private final CriteriaQuery<?> query = mock(CriteriaQuery.class);
	private final CriteriaBuilder cb = mock(CriteriaBuilder.class);

	@Test
	void fromFilters_sinFiltros_noAgregaPredicados() {
		ClientFilterParams filtros = new ClientFilterParams();
		Predicate combinado = mock(Predicate.class);
		ArgumentCaptor<Predicate[]> captor = ArgumentCaptor.forClass(Predicate[].class);
		when(cb.and(captor.capture())).thenReturn(combinado);

		Predicate resultado = ClientQueryBuilder.fromFilters(filtros).toPredicate(root, query, cb);
	...
	void fromFilters_conSearchEnBlanco_noAgregaPredicado() {
		...
		ClientQueryBuilder.fromFilters(filtros).toPredicate(root, query, cb);
